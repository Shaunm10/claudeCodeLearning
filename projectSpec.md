...
4. Database & Migrations
   ...
   TEXT
   FOREIGN KEY
   References the users table from better-auth.
   content
   TEXT
   NOT NULL
   The TipTap rich text data stored as a stringified JSON object.
   is_shared
   INTEGER
   DEFAULT 0
   Boolean flag (0 = private, 1 = public).
   created_at
   INTEGER
   NOT NULL
   Unix timestamp.
...
5. API Route Design (/app/api/...)
   ...
   POST
   Yes
   Creates a new note. Body: { content: string }.
   /api/notes/[id]
   GET
   Yes
   Retrieves a specific note for the author for editing.
   /api/notes/[id]
   PUT
   Yes
   Updates a note's content or is_shared status.
   /api/notes/[id]
   DELETE
   Yes
   ...
   GET
   No
   Retrieves a note only if its is_shared flag is 1 (true).
